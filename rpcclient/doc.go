...
In addition to the compatible standard HTTP POST JSON-RPC API, pktd and
pktwallet provide a websocket interface that is more efficient than the standard
...
pktd or pktwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by pktd and
pktwallet only uses a single connection that remains open and allows
...
All notifications provided by pktd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by pktd (and
pktwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that pktd intentionally separates the wallet
functionality into a separate process named pktwallet.  This means if you are
connected to the pktd RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, pktwallet provides pass through treatment
...
The following full-blown client examples are in the examples directory:
 ...
 - pktdwebsockets
   Connects to a pktd RPC server using TLS-secured websockets, registers for
   block connected and block disconnected notifications, and gets the current
   block count
 - pktwalletwebsockets
   Connects to a pktwallet RPC server using TLS-secured websockets, registers
