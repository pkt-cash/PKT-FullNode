...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-pktd.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "pktd")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of pktd binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-pktd.conf")
